...
const Resources = () => {
  ...
  const hasPrev = activeResource && activeTopicIndex !== null && (activeSubtopicIndex > 0 || activeTopicIndex > 0);
    ...
    <div className="min-h-screen bg-dark-bg pt-2 pb-20 font-sans">
      <div className="max-w-[1280px] mx-auto px-5">
        ...
        <div className="bg-dark-card rounded-[2.5rem] p-5 shadow-card border border-dark-border z-10 relative">
          ...
          <div className="flex justify-center mb-8 border-b border-gray-200">
            <button
              className={`flex items-center gap-2 px-6 py-3 font-black text-lg transition-colors border-b-2 ${
                activeTab === "roadmap"
          ...
          {activeTab === "roadmap" && !activeResource && (
            <div className="max-w-4xl mx-auto">
              ...
              {/* Empty State */}
              {!loading && !error && (
                <div className="text-center py-10 px-4 border-2 border-dashed border-dark-border rounded-3xl bg-dark-bg">
                  <div className="w-24 h-24 rounded-[2rem] flex items-center justify-center mx-auto mb-6 bg-cyan-500/10 text-cyan-500">
              ...
              {loading && (
                <div className="text-center py-20">
                  ...
                  <h3 className="text-xl font-black text-slate-800 mb-2">Architecting your roadmap...</h3>
                  <p className="text-slate-500 font-semibold">
                    Analyzing the best learning path for {selectedSkill || "your topic"}.
                  </p>
                </div>
              )}
            </div>
          ...
          {activeTab === "roadmap" && activeResource && (
            <div className="max-w-5xl mx-auto">
              ...
              <div className="flex flex-col md:flex-row justify-between items-center mb-10 pb-6 border-b border-gray-100">
                <div className="flex-1">
                  <button
                    ...
                    className="text-sm text-slate-500 hover:text-slate-800 font-black mb-3 flex items-center gap-2 uppercase tracking-widest"
                  >
                    &larr; Build New Roadmap
                  </button>
                  <h2 className="text-3xl font-extrabold text-slate-900 mb-2 tracking-tight">{activeResource.skill}</h2>
                  <span className="inline-flex items-center px-4 py-1.5 rounded-xl text-xs font-black uppercase tracking-widest bg-cyan-500/10 text-cyan-600">
              ...
              <div className="relative pl-4 md:pl-8">
                {/* Vertical track line */}
                <div className="absolute left-[39px] md:left-[55px] top-6 bottom-6 w-1 bg-dark-border rounded-full z-0"></div>

                {activeResource.roadmapData &&
                  activeResource.roadmapData.map((topic, tIndex) => (
                      ...
                      <div className="pl-14 md:pl-[4.5rem] flex flex-col gap-4">
                        {topic.subtopics.map((subtopic, sIndex) => (
                          <div
                            ...
                            className={`group relative flex items-center p-5 rounded-[1.5rem] border-2 transition-all duration-300 cursor-pointer hover:-translate-y-1 hover:shadow-[0_15px_30px_rgba(6,182,212,0.08)]
                                            ${subtopic.completed ? "bg-emerald-50/50 border-emerald-200" : "bg-dark-card border-dark-border hover:border-cyan-500/50 shadow-card"}`}
                          >
                            {/* Checkbox */}
                            <div
                            ...
                            >
                              {subtopic.completed ? (
                                <FaCheckCircle className="text-3xl text-emerald-500 hover:text-emerald-600 transition-colors" />
                              ) : (
                                <div className="w-7 h-7 rounded-full border-2 border-slate-300 group-hover:border-cyan-500 transition-colors flex items-center justify-center bg-white cursor-pointer"></div>
                              )}
                            </div>

                            <div className="flex-grow">
                              <h4
                                className={`text-lg font-black transition-colors tracking-tight ${subtopic.completed ? "text-slate-400 line-through" : "text-slate-800 group-hover:text-cyan-500"}`}
                              >
                                {subtopic.title}
                              </h4>
                            </div>
                            ...
                            <div className="text-[9px] uppercase font-black tracking-widest px-4 py-2 rounded-xl bg-slate-100 text-slate-500 hidden sm:block group-hover:bg-cyan-500 group-hover:text-white transition-colors">
                ...
                <div className="relative z-10 mt-16 pt-8 border-t-2 border-dashed border-dark-border">
                  <div className="flex items-center justify-center mb-6">
                    <div
                      className={`w-16 h-16 rounded-full flex items-center justify-center shadow-lg border-4 border-white z-10 text-2xl text-white ${isTestUnlocked ? "bg-gradient-to-br from-amber-400 to-orange-500" : "bg-slate-300"}`}
                    >
                  ...
                  <div className="text-center">
                    <h3
                      className={`text-2xl font-black tracking-tight mb-2 ${isTestUnlocked ? "text-slate-900" : "text-slate-400"}`}
                    >
                      Final AI Assessment
                    </h3>
                    <p
                      className={`mb-6 max-w-lg mx-auto font-semibold ${isTestUnlocked ? "text-slate-600" : "text-slate-400"}`}
                    >
                    ...
                    {activeResource?.testData?.status === "completed" ? (
                      <button
                        ...
                        className="px-8 py-3 bg-indigo-600 text-white font-black text-[10px] uppercase tracking-widest rounded-xl shadow-lg hover:bg-indigo-700 transition flex items-center justify-center gap-2 mx-auto"
                      >
                      ...
                      <button
                        ...
                        className={`px-8 py-4 text-[10px] uppercase font-black tracking-[0.25em] rounded-[1.2rem] transition-all flex items-center justify-center gap-2 mx-auto
                              ${isTestUnlocked ? "bg-cyan-900 text-white hover:bg-cyan-500 shadow-xl shadow-cyan-900/20 hover:shadow-cyan-500/30" : "bg-dark-card border border-dark-border text-slate-400 cursor-not-allowed shadow-none"}`}
                      >
              ...
              {historyLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {historyData.length === 0 ? (
                    <div className="col-span-full text-center py-16 border-2 border-dashed border-dark-border rounded-[2.5rem] bg-dark-bg">
                      <FaHistory className="text-4xl text-slate-300 mx-auto mb-4" />
                      <h3 className="text-xl font-black text-slate-900">No roadmaps found</h3>
                      <p className="mt-2 text-slate-500 font-semibold text-sm">
                        Your generated paths will appear here.
                      </p>
                    </div>
                  ) : (
                    historyData.map((item) => {
                      ...
                      return (
                        <div
                          ...
                          className="bg-dark-card p-8 rounded-[2.5rem] border border-dark-border shadow-card hover:shadow-[0_20px_50px_rgba(6,182,212,0.12)] transition-all duration-500 cursor-pointer flex flex-col h-full relative overflow-hidden group"
                        >
                          ...
                          <div className="flex justify-between items-start mb-6 z-10 relative">
                            <span
                              className={`px-3 py-1.5 text-xs font-bold uppercase tracking-wider rounded-lg bg-gray-100 text-gray-600`}
                            >
                              {item.skill}
                            </span>
                          ...
                          <h3 className="text-2xl font-black text-slate-800 mb-2 z-10 relative group-hover:text-cyan-500 transition-colors line-clamp-2 tracking-tight">
                            {item.skill} Roadmap
                          </h3>
                          <p className="text-xs font-black text-slate-400 mb-6 z-10 relative flex items-center gap-2 uppercase tracking-widest">
                            <FaCalendar className="text-slate-300" /> Target: {item.timeframe}
                          </p>

                          <div className="mt-auto z-10 relative pt-4 border-t border-dark-border">
                            <div className="flex justify-between text-[10px] font-black tracking-widest text-slate-400 mb-2">
                              <span>PROGRESS</span>
                              <span className={percent === 100 ? "text-emerald-500" : "text-cyan-500"}>{percent}%</span>
                            </div>
                            <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                              <div
                                className={`h-full transition-all duration-500 ${percent === 100 ? "bg-emerald-500" : "bg-cyan-500"}`}
                                style={{ width: `${percent}%` }}
                              ></div>
                            </div>
                          </div>
      ...
      {modalOpen && activeResource && activeTopicIndex !== null && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 font-sans">
          ...
          {/* Modal Built */}
          <div className="relative bg-dark-card w-full max-w-5xl max-h-[90vh] rounded-[2.5rem] shadow-[0_30px_60px_rgba(0,0,0,0.2)] border border-dark-border flex flex-col overflow-hidden animate-fade-in">
            {/* Modal Header */}
            <div className="flex items-center justify-between p-6 sm:p-8 border-b border-dark-border bg-dark-bg">
              <div className="flex-1 pr-6">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">
                  {activeResource.roadmapData[activeTopicIndex].title}
                </p>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="hidden sm:flex items-center gap-2 px-6 py-2 bg-slate-900 text-white text-[10px] uppercase font-black tracking-widest rounded-xl hover:bg-slate-800 transition-colors shadow-md"
                >
                  <FaDownload /> Download Note
                </button>
                <button
                  onClick={(e) => toggleSubtopicCompletion(activeTopicIndex, activeSubtopicIndex, e)}
                  className={`flex items-center gap-2 px-6 py-2 text-[10px] uppercase font-black tracking-widest rounded-xl transition-all border-2
                                ${
                                  activeResource.roadmapData[activeTopicIndex].subtopics[activeSubtopicIndex].completed
                                    ? "bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100"
                                    : "bg-white text-slate-600 border-dark-border hover:border-slate-300 hover:bg-slate-50"
                                }`}
                >
                  <FaCheckCircle
                    className={
                ...
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center rounded-full bg-white border border-dark-border text-slate-400 hover:text-red-500 hover:bg-red-50 hover:border-red-200 transition-colors ml-2"
                >
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-10 scroll-smooth">
              {noteLoading ? (
                <div className="flex flex-col items-center justify-center h-full py-20 text-center">
                  <FaSpinner className="animate-spin text-5xl text-cyan-500 mb-6" />
                  <h3 className="text-xl font-black text-slate-800 mb-2">Generating Comprehensive Note...</h3>
                  <p className="text-slate-500 max-w-md font-semibold">
                    Our AI tutor is writing a detailed, custom note for{" "}
                    {activeResource.roadmapData[activeTopicIndex].subtopics[activeSubtopicIndex].title}. This will be
                    permanently saved to your roadmap.
                  </p>
                </div>
              ) : (
            ...
            <div className="border-t border-dark-border p-6 sm:p-8 bg-dark-bg flex items-center justify-between">
              <button
                onClick={handlePrevNote}
                disabled={!hasPrev || noteLoading}
                className="flex items-center gap-2 px-6 py-4 rounded-[1.2rem] text-[10px] uppercase font-black tracking-widest transition-all disabled:opacity-50 disabled:cursor-not-allowed text-slate-500 bg-white border border-dark-border hover:bg-slate-50 hover:text-slate-900 shadow-sm"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-8 py-4 bg-cyan-900 text-white text-[10px] uppercase font-black tracking-[0.25em] rounded-[1.2rem] hover:bg-cyan-500 hover:shadow-cyan-500/30 transition-all shadow-xl shadow-cyan-900/20 disabled:opacity-50 disabled:cursor-not-allowed"
              >
      ...
      {testInstructionsOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 font-sans">
          ...
          ></div>
          <div className="relative bg-dark-card w-full max-w-2xl rounded-[2.5rem] shadow-[0_30px_60px_rgba(0,0,0,0.2)] border border-dark-border p-8 sm:p-12 scroll-smooth animate-fade-in text-center">
            <div className="w-20 h-20 rounded-full bg-amber-50 text-amber-500 flex items-center justify-center mx-auto mb-6 shadow-inner">
              <FaExclamationTriangle className="text-3xl" />
            </div>
            <h2 className="text-3xl font-black text-slate-900 mb-6 tracking-tight">Are you ready?</h2>
            <div className="text-left bg-dark-bg p-8 rounded-[1.5rem] border border-dark-border mb-10 space-y-4 text-slate-600 font-semibold tracking-wide leading-relaxed">
              <p>
                <strong className="text-slate-900 font-black">1. One Attempt Only:</strong> Once you click start, you
                cannot retake this test. Leaving the page will submit your current progress.
              </p>
              <p>
                <strong className="text-slate-900 font-black">2. Timed Environment:</strong> You will have exactly 30
                minutes to complete the test. It will auto-submit when the timer reaches 00:00.
              </p>
              <p>
                <strong className="text-slate-900 font-black">3. Comprehensive:</strong> Expect 10 Multiple Choice
                Questions (and 2 Coding Questions if the topic is technical).
              </p>
              <p className="text-[10px] uppercase font-black tracking-widest text-red-500 pt-2">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button
                onClick={() => setTestInstructionsOpen(false)}
                className="px-8 py-4 rounded-[1.2rem] text-[10px] uppercase font-black tracking-widest text-slate-500 hover:text-cyan-900 transition-all bg-dark-bg hover:bg-white border border-transparent hover:border-dark-border shadow-sm hover:shadow-md"
              >
                Cancel
              </button>
              <button
                ...
                className="px-10 py-4 bg-cyan-900 text-white text-[10px] uppercase font-black tracking-[0.25em] rounded-[1.2rem] hover:bg-cyan-500 transition-all shadow-lg hover:shadow-xl disabled:opacity-50 flex items-center justify-center gap-2"
              >
      ...
      {testActiveOpen && activeResource?.testData?.questions && (
        <div className="fixed inset-0 z-[100] flex flex-col bg-dark-bg font-sans animate-fade-in">
          {/* Header */}
          <div className="sticky top-0 bg-white/90 backdrop-blur-md border-b border-dark-border p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-center z-10 shadow-sm gap-4">
            <div>
              <h2 className="text-2xl font-black text-cyan-900 tracking-tight">
                {activeResource.skill} Final Assessment
              </h2>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
              <div
                className={`text-xl font-mono font-black px-6 py-3 rounded-[1.2rem] shadow-inner ${testTimeLeft < 300 ? "bg-red-50 text-red-600 animate-pulse border border-red-100" : "bg-slate-50 border border-dark-border text-slate-600"}`}
              >
              ...
              <button
                ...
                disabled={testLoading}
                className="w-full sm:w-auto px-10 py-4 bg-cyan-500 text-white text-[10px] uppercase font-black tracking-widest rounded-xl hover:bg-cyan-900 transition-all shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
              >
            ...
            <div className="max-w-4xl mx-auto space-y-12 pb-20">
              ...
              {activeResource.testData.questions.mcqs && activeResource.testData.questions.mcqs.length > 0 && (
                <div className="space-y-8">
                  <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 border-b border-dark-border pb-3">
                    Multiple Choice Questions
                  </h3>
                  {activeResource.testData.questions.mcqs.map((q, idx) => (
                    <div key={q.id} className="bg-dark-card p-8 rounded-[2rem] border border-dark-border shadow-card">
                        ...
                        {q.options.map((opt, optIdx) => (
                          <label
                            ...
                            className={`flex items-center gap-4 p-5 rounded-[1.5rem] cursor-pointer transition-all duration-200 border-2
                                         ${testAnswers.mcqs[q.id] === optIdx ? "bg-cyan-500/5 border-cyan-500 shadow-md" : "bg-dark-bg border-transparent hover:border-slate-200 hover:bg-white"}`}
                          >
                            <input
                              type="radio"
                              name={`mcq-${q.id}`}
                            ...
                            />
                            <span
                              className={`font-black tracking-tight ${testAnswers.mcqs[q.id] === optIdx ? "text-cyan-900" : "text-slate-600"}`}
                            >
                              {opt}
                            </span>
                          </label>
                ...
                <div className="space-y-8 pt-8">
                  ...
                  {activeResource.testData.questions.coding.map((q, idx) => (
                    <div key={q.id} className="bg-dark-card p-8 rounded-[2rem] border border-dark-border shadow-card">
                      ...
                      <textarea
                        className="w-full h-56 p-6 font-mono text-sm bg-slate-900 text-cyan-400 rounded-[1.5rem] focus:ring-4 focus:ring-cyan-500/20 focus:border-cyan-500 outline-none shadow-inner resize-y transition-all"
      ...
      {testResultsOpen && activeResource?.testData?.status === "completed" && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 font-sans">
          ...
          ></div>
          <div className="relative bg-dark-card w-full max-w-3xl rounded-[2.5rem] shadow-[0_30px_60px_rgba(0,0,0,0.2)] overflow-hidden animate-fade-in flex flex-col max-h-[90vh] border border-dark-border">
            {/* Header / Score Section */}
            <div className="p-6 md:p-8 text-center bg-gradient-to-br from-cyan-900 to-cyan-500 text-white relative shrink-0">
              <button
                onClick={() => setTestResultsOpen(false)}
              ...
              <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8">
                ...
                <div className="text-center md:text-left flex-1">
                  ...
                  <p className="text-cyan-100 font-black text-[10px] md:text-xs tracking-widest uppercase">
                    Score based on MCQs and AI-evaluated logic.
                  </p>
                </div>
            ...
            <div className="flex-1 p-6 md:p-10 overflow-y-auto bg-dark-bg">
              <h3 className="text-[10px] sm:text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 border-b border-dark-border pb-2 mb-6">
                AI Feedback & Analytics
              </h3>
              <div className="prose prose-sm md:prose-base lg:prose-lg prose-cyan max-w-none prose-headings:font-black prose-p:font-semibold prose-strong:font-black text-slate-700">
                <ReactMarkdown remarkPlugins={[remarkGfm]}>{activeResource.testData.analytics}</ReactMarkdown>
              </div>
            </div>
            ...
            <div className="p-6 border-t border-dark-border bg-dark-card shrink-0">
              <button
                onClick={() => setTestResultsOpen(false)}
                className="w-full md:w-auto md:px-12 py-4 bg-cyan-900 text-white text-[10px] uppercase font-black tracking-widest rounded-xl hover:bg-cyan-500 hover:shadow-xl transition-all shadow-md mx-auto block"
              >
